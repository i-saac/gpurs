...
pub struct MemoryHandler<T: IsFloat + std::fmt::Debug + Copy + Clone> {
    ...
    write_buffers: Vec<Buffer<T>>, // Vector of full write buffers
    last_write_event: Option<Event> // Last write event object
}

impl<T: IsFloat + std::fmt::Debug + Copy + Clone> MemoryHandler<T> {
...
impl MemoryHandler<f32> {
    pub fn init(program_source: &str, kernel_names: Vec<&str>) -> Result<MemoryHandler<f32>> {
        ...
        let output: MemoryHandler<f32> = MemoryHandler {
            context: context,
            command_queue: queue,
...
impl MemoryHandler<f64> {
    pub fn init(program_source: &str, kernel_names: Vec<&str>) -> Result<MemoryHandler<f64>> {
        // Get devices and create device object
        let device_id = *get_all_devices(CL_DEVICE_TYPE_GPU)?
            .first()
            .expect("No device found in platform");
        ...
        // Create and return new Memory Handler
        let output: MemoryHandler<f64> = MemoryHandler {
            context: context,
            command_queue: queue,
            kernels: kernel_vector,
            write_buffers: buffer_vector,
    ...
    ) -> Result<(Matrix<f64>, usize)>
    {
        ...
        // Collect selected write buffers into separate vector
        let selected_write_buffers: Vec<&Buffer<f64>> = input_mat_idcs
            .iter()
            .map(|&idx| &self.write_buffers[idx])
            .collect::<Vec<_>>();
