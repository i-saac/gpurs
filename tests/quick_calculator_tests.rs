...
fn memory_gpu_matmul_test() {
    ...
    let c_mat: Matrix = calc.quick_mat_mul(&a_mat, &b_mat)
    ...
    let d_mat: Matrix = calc.halfquick_mat_mul(b_idx, &c_mat)
...
fn quick_custom_kernel_test() {
    ...
    let custom_param_function: QuickParameterFunction = Box::new(
        | input_stored_mats: Option<Vec<&Matrix>>, input_temp_mats: Option<Vec<&Matrix>> |
